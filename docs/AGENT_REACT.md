...
- 版: v0.1（2026-07-18, Fable 起草 — 利用者との議論合意を仕様化。**PoC 未実施**のため §7 の
  数値パラメータは暫定。P1〜P4（§6）の実測を受けて Fable が確定版 v1.0 に改訂してから実装に入る）
...
- 両筐体で vLLM / CUDA / モデル重み（HF キャッシュ）のバージョン・内容を一致させる。
  ...
  層分割の偏りは `VLLM_PP_LAYER_PARTITION` で調整可（P1 の実測ノブ）。
- 目標値（P1 で実測確定）: `max-model-len ≥ 16384`・`max-num-seqs ≥ 4`・prefix caching 有効。
...
- PP=2 は 1 論理インスタンスであり、**nubia 断・LAN 断・Ray 断で生成が全停止**する。
  P1 成果物に死活確認と再起動手順書（利用者実施部分を明示）を含めること。
...
- **昇格候補: native tool calling** — vLLM の Gemma 4 用 tool-call パーサが P2 で安定と判明した
  場合のみ主へ昇格（並列ツール呼び出し等の利得）。判定は P2 で短時間で見切る。
...
| 項目 | 暫定値 | 確定根拠 |
|---|---|---|
| soft 閾値 | 実効コンテキスト窓の 70% | P1 の実効窓と P4 の観測サイズ実測 |
| hard 閾値 | 同 85% または evidence ⊇ generate 実予算 | 同上 |
| 観測 1 件の上限 | 120 トークン級（タイトル＋2 行要約） | P3/P4 |
| navigator 内部 decide | 上限 2〜3 手 | P3 |
| decide 温度・max_tokens | 0.2 / 300（現行 evaluate 踏襲） | P2/P3 |
| recursion_limit | 50（FR-33 踏襲） | — |
...
- 直列: **FR-33 本番反映（利用者）→ P1 nubia 構築（Codex 準備・利用者実施）→ P2〜P4 PoC（Codex）→
  §7 確定・v1.0 改訂（Fable）→ 実装（Codex, `feature/fr-34-react-harness` → develop PR）→ 検収（Fable）**。
