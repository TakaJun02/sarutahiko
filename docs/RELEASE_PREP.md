...
- 版: v0.5（2026-07-17, Fable 改訂 — FR-22 改訂: About モーダルの並び順を
  「本文 → 研究室サイトリンク → QR セクション」へ変更（利用者指示）。§11-1/11-2/11-4 更新。
  あわせて §11-2 項 5 の注記が利用者直編集（81646cd）で削除済みであることを追記）
- v0.4（2026-07-15, Fable 改訂 — FR-25: 回答ストリーミングの「なめらか文字送り」。§14 追加。
  あわせて対象 FR 索引に FR-23/24 の記載漏れを補記）
- v0.3（2026-07-15, Fable 改訂 — FR-22: About モーダルに APU-Navi アクセス用 QR コードを追加。
...
- 対象 FR: **FR-18**（`docs/SPEC.md` v0.10）・**FR-19**（同 v0.11、§8）・**FR-20**（同 v0.12、§9）・
  **FR-21**（同 v0.13、§10）・**FR-22**（同 v0.14、§11）・**FR-23**（同 v0.15、§12）・
  **FR-24**（同 v0.16、§13）・**FR-25**（同 v0.17、§14）
...
### 11-1 配置（2026-07-17 改訂・利用者指示）

- 挿入位置: About モーダル（`dialog.kind === 'about'`）内、**研究室サイトリンクの直後・
  閉じるボタンの直前**（モーダル全体の並びは「本文 → 研究室サイトリンク → QR セクション」）。
  ※ v0.3 当初は「本文の直後・研究室サイトリンクの直前」だったが、2026-07-17 利用者指示で
  QR セクションを末尾へ移動。
- 上下を髪線 divider（`border-edge` 相当）または十分な余白で区切り、
  「本文 / 研究室リンク / QR セクション」が別ブロックと読めるようにする（手段は Sol 裁量）。
...
5. ~~注記（※ 研究室ホームページの QR コードではありません…）~~ —
   利用者直編集（81646cd）で削除済み。2026-07-17 の並び替え（QR がリンクの後ろへ移動）により
   「すぐ下のリンク」前提も消滅したため、復活させない。
...
- [ ] About モーダルが「本文 → 研究室サイトリンク → QR セクション」の順（2026-07-17 改訂）で表示され、
      QR セクション内の文言・並び順が §11-2 と一致。
...
- **R2（2026-07-15 Fable 検収指摘・Playwright 実測に基づく計測方式の是正）**:
  R1 の isAtBottom 計測（絶対最下端基準 72px ＋ 600ms 抑制窓）に実測で欠陥が出た。
  (a) `scrollToBottom` の着地点は **sticky footer の in-flow 高さぶん**（実測 106px）絶対最下端より
  上にある（composer は sticky でスクロール内容としての高さを持つため）。72px 閾値では**正常な着地点
  自体が「離脱」と誤判定**され、送信後に自動追従が切れ・キーボードピン留め直後にボタンが誤表示された。
  (b) 600ms 抑制窓はアニメーション長依存のマジックナンバーで、長距離 smooth の尾イベントが漏れる。
  次の 3 点セットで置換する:
  1. 計測は**実効ギャップ** `scrollHeight - scrollTop - clientHeight - (footerRef.offsetHeight || 0)
     <= 72` で判定する（sticky footer の in-flow 高さを除いた「見た目の最下部」基準）。
  2. **離脱（false 化）は上方向スクロールのときのみ**: 実効ギャップ > 72 かつ scrollTop が直前
     イベントより減少した場合に限る（下向きのプログラムスクロールの途中経過・中断では離脱しない。
     直前値はモジュール変数 `lastScrollTop` に保持し毎イベント更新）。実効ギャップ <= 72 なら常に true。
  3. `SMOOTH_SCROLL_SUPPRESS_MS` と時間ベース抑制は**撤去**（2. により不要）。
     `scrollToBottom()` が実行後に `isAtBottom = true` を設定する点は維持。
  テストも R2 semantics に合わせて更新する（footer 差し引き式・上方向限定の離脱・suppress 定数の不存在）。
...
## 14. FR-25 回答ストリーミングの「なめらか文字送り」（2026-07-15 追加・利用者指示）

**利用者指示（2026-07-15）**: バックから AI の出力を受け取ってフロントに表示する際、ストリーミング出力にすること。

### 14-1 背景と計測事実（実装前に Fable が 3 経路で実測）

- バックエンド直結（`POST /api/chat`）: token イベントは**逐次到着**
  （実測 132 トークン / 1.87s、平均間隔 14.3ms ≈ vLLM 70 tok/s。一括到着ではない）。
- 本番 nginx 経由（ポート 80、`proxy_buffering off` 設定済み）: 同様に逐次到着（平均 14.2ms）。
- 実ブラウザ（/var/www/dist の本番ビルド、Playwright）: DOM 上の回答文字数は 250ms
  サンプリングで毎回 20〜40 文字ずつ単調増加 — **逐次描画も機能している**。
- したがって **backend / nginx / SSE / store のトークン逐次配信に欠陥はない。backend は変更しない。**
- 一方で体感は「長い無音（TTFT 実測 6〜35s、エージェントループ＋プリフィル由来）→
  約 100〜140 文字/s で一気に流れて数秒で完了」。到着がネットワーク事情でバースト化すると
  「一括表示」に見え得る。**表示側にペーシングがないことが体感問題の本質**。

### 14-2 方針

- 到着済みテキストをフロントで**一定の時定数で平滑化して文字送り表示**する
  （ChatGPT / Gemini アプリと同じクライアント側ペーシング）。
  どんな到着パターン（バースト・停滞）でも「常に流れて見える」ことを保証する。
- FR-3 の「参考実装の単語ごと 40ms 表示は不採用」の裁定は、**完成済み回答の擬似タイプライター**の
  否定であり、本件（実ストリーミングの表示平滑化）とは別物。FR-3 の実ストリーミング原則は維持し、
  SPEC.md v0.17 で FR-3 に本節への参照を追記する。
- 変更は **frontend のみ**。SSE スキーマ・backend・nginx は不変。

### 14-3 表示モデル（store）

- `createMessage()` に `revealedLength`（表示済み文字数）を追加。
  **既定値は `content.length`**（履歴復元・ユーザー発話・エラー文は常に全文即表示）。
  送信時の assistant ドラフトは `content: ''` なので 0 から始まる。
- `applyAssistantEvent`:
  - `token`: 従来どおり `content += text`。`revealedLength` は触らない（ティッカーが進める）。
  - `done`: `thread_id` の返却・`id` 確定は従来どおり**即時**。ただし `streaming=false` と
    `sources` 反映は即時に行わず、`doneReceived = true`・`finalSources = event.data.sources` を
    メッセージに記録して**文字送り完了時に確定**させる（下記 finalize）。
    出典 UI・settled 遷移が本文の文字送り完了より先に出ないための措置。
  - `error`: 従来どおり即時（content 差し替え）。`revealedLength = content.length` に snap。
- **finalize**（ティッカー側で実行）: `doneReceived && revealedLength === content.length` になったら
  `streaming = false`・`sources = finalSources` を反映し、一時フィールドを片付ける。
- 表示: ChatView は MarkdownRenderer に `message.content.slice(0, message.revealedLength)` を渡す。
  `revealedLength >= content.length` のときは全文（現行と同一の見え方）。

### 14-4 文字送りティッカー

- rAF ベースの**単一ループ**。文字送りが必要なメッセージ（`streaming` かつ
  `revealedLength < content.length`、または finalize 待ち）が存在する間だけ回し、
  なくなったら停止する（アイドル時の常時 rAF 禁止）。ChatView unmount / `newChat` /
  `openThread` で対象が消えたら自然停止すること（store の現在の messages だけを見る）。
- 毎フレームの進行量は**純関数**に切り出す（ユニットテスト対象):
  `advanceRevealCount({ revealed, total, dtSeconds }) => newRevealed`
  - `backlog = total - revealed`（0 なら進めない）
  - `rate = max(MIN_REVEAL_RATE, backlog / DRAIN_SECONDS)`（chars/s）
  - `advance = min(backlog, max(1, round(rate * dtSeconds)))`
  - `dtSeconds` は **0.1s で clamp**（フレーム落ち・タブ切替直後の巨大ジャンプ防止）
- 定数（frontend の定数として定義。値は検収で微調整可、初期値は以下）:
  - `MIN_REVEAL_RATE = 40`（chars/s。読める下限速度）
  - `DRAIN_SECONDS = 0.9`（追いつき時定数。backlog は指数的に減衰し、到着が表示より速くても
    表示遅延は概ね backlog ≈ 到着レート × 0.9s に収束する。done 受信後の掃き切りは
    指数減衰＋MIN_RATE 床のため**典型 ≈2s・大バースト（400 字級一括到着）で ≈3s**
    — 検収実測 2026-07-15: 407 字バーストを 2.8s で平滑表示。これは「常に流れて見える」という
    本 FR の目的そのものであり、遅延ではなく演出時間として許容する）
- **snap（即時全文表示）にする場合**:
  - `error` 到着時・送信 catch 時
  - `prefers-reduced-motion: reduce`（ペーシング自体を無効化。FR-17 以来の方針と整合）
  - `visibilitychange` で hidden → visible に復帰したとき（背景タブでは rAF が止まり
    backlog が溜まるため、復帰時に streaming 中メッセージを snap してから通常運転に戻す）

### 14-5 既存挙動との整合

- LoadingSpinnerV5 の `pending → settled`（初回 token で本文領域へ切替）は**不変**。
- `isSending`（composer ゲート・busyHint）は従来どおり **SSE 終了で解除**してよい
  （drain は最大 ≈1s なので二重送信リスクなし）。
- FR-24 規則 2 の自動追従: 追従トリガーのメッセージ内容シグネチャに**表示中テキスト長
  （revealedLength）の変化が含まれる**こと（per-frame 成長でも最下部ピン留めが滑らかに働き、
  上に遡って読んでいる間は引き戻さない — 現行セマンティクスの維持）。
- Markdown の再パースは表示文字列が変わったフレームのみ（computed の入力を slice にすれば自然に
  そうなる）。それ以上の最適化（差分レンダラ等）は本 FR ではやらない。

### 14-6 テスト

- `advanceRevealCount` のユニットテスト:
  (a) backlog 小: 1 文字以上ずつ単調増加し `total` を超えない
  (b) backlog 大（例 900 文字）: 約 DRAIN_SECONDS で掃き切れる進行になる
  (c) `dtSeconds` clamp: 大きな dt（例 5s）でも 1 フレームの進行が clamp 相当に収まる
  (d) backlog 0: 進行 0
- store テスト（`chat.spec.js` 形式）:
  (e) `done` 受信時点では sources 未反映・streaming のまま → revealedLength が全文に達した
      finalize で streaming=false・sources 反映
  (f) `error` で snap（revealedLength == content.length）
  (g) 履歴復元（openThread 相当）のメッセージは revealedLength == content.length
- 既存テストに回帰なし。`npm run test` / `npm run build` green。

### 14-7 検収チェックリスト（Fable）

- [ ] Playwright 実測: 生成開始〜完了の表示文字数を 100ms 粒度でサンプリングし、単調増加かつ
      増分が安定（>300ms の表示停滞が backlog 存在中に発生しない）。
- [ ] done 到着から表示完了(全文一致)まで、通常ストリームで ≤ 約2s・大バースト一括到着でも ≤ 約3s
      （§14-4 の指数減衰モデルどおり）。出典・settled 遷移は表示完了後に出る。
- [ ] トークンをバースト到着させても（開発者ツールでネットワーク throttle）表示は滑らかに流れる。
- [ ] FR-24: 文字送り中の最下部ピン留め・上スクロールでの追従停止・「最新へ戻る」復帰が機能。
- [ ] reduced-motion 環境で従来どおり到着即表示。
- [ ] エラー時は全文即時表示。タブ非表示→復帰で表示が破綻しない。
- [ ] `npm run test` / `npm run build` green。
