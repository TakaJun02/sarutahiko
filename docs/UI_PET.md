...
>
> v1.3（2026-07-19 利用者指示）: ①名称確定 みちかぜ→**猿田彦**・ガオウ→**なまはげ**（§4-3）
> ②各フォームに**図鑑説明文**を新設（§4-3）③About ヒントボタンを分岐: 仲間がいない時=従来ヒント／
> いる時=**ペットずかん**（「特別な機能はない・癒し担当」＋ 6 体の説明。§3-2・§12-9）
> ④選択 UI を**枠なしの「顕現」**へ再々設計 — カード/枠/ヘッダの「フォーム骨格」を全廃し、
> 日輪ハロー＋地明かりの上に 6 体が煙ポップで順に現れる（§12-11 全面改稿・意匠リード Fable）。
>
> v1.4（2026-07-19 利用者指示）: **画面上のペットをタップすると選び直しの顕現が開く**（§3-1・§6-3。
> タップ経路は待機演出・一時バブルなしで顕現のみ。リアクションは併発）。
...
- **ペットタップ経路（v1.4）**: 召喚済みペットへのシングルタップ（§6-3）でも顕現を開く。
  こちらは**直接操作なので待機演出（バブル・スピナー・1.9 秒）を挟まず**、`picking` へ直行して
  顕現アニメのみ再生（一時ブロックにユーザーバブルは出さない）。開けるのは「通常送信が可能な状態」のみ
  （`isSending`・FR-27 origin・FR-39 clarification 中はリアクションのみ）。開いたら最下部へスクロールし、
  composer ロック・解除条件・履歴ゼロは合言葉経路と同一。
...
- 押下しても**召喚はしない**。表示内容は状態で分岐する（v1.3）:
  - **仲間がいない時**（`currentForm` なし）: ボタン右のヒント 1 行をトグル表示:
    **「メッセージで「ペットを呼び出す」と送ってみよう！」**（従来どおり）。
  - **仲間がいる時**（`unlocked && currentForm`・表示トグルの ON/OFF は問わない）: **「ペットずかん」**
    （§12-9）をトグル表示する。内容 = 機能説明「このコたちに特別な機能はありません。となりで一緒に
    待ってくれる、癒し担当です。」＋ 6 体の名前と図鑑説明文（§4-3）。
  - いずれも再押下で非表示・`aria-expanded` 連動・ダイアログを閉じたら次回は非表示から。
...
| id | 名（仮） | モチーフ | 軸 | 枠 |
|---|---|---|---|---|
| `robo` | ぴこ | 利用者支給リファレンス `referenceUI/piko.png`（ローカル保持・コミット禁止）準拠。スクリーン顔＋耳フィン＋胸ランプの人形型ロボット | かわいい | 通常 |
| `sarutahiko` | 猿田彦 | 利用者支給リファレンス `referenceUI/sarutahiko.webp` 準拠。長鼻・白いたてがみ・冠・注連縄・尾を持つ獣神サルタヒコの子（**かっこいい**軸・経路案内アプリと符合） | かっこいい | 通常 |
| `akita` | こまち | 秋田犬の子犬 | かわいい | 通常 |
| `gotenmari` | てまりん | 本荘ごてんまり（伝統工芸のまり）がまんまるに跳ねるペット | かわいい | 通常 |
| `namahage` | なまはげ | 利用者支給リファレンス `referenceUI/namahage.jpg` 準拠。青系の面・荒れたたてがみ・蓑・桶（刃物は不採用）の**かっこいい**軸 | かっこいい | **レア** |
...
- 軸は利用者検収（2026-07-19）で確定: かわいい軸 3 体＋かっこいい軸 3 体。どちらの軸でも
  ...
  キャラ本体に限り UI トークン外の専用色を許可（利用者指示 2026-07-19・§12-2 R7 パレット）。
  名前は **v1.3 で利用者確定**（§4-3。みちかぜ→猿田彦・ガオウ→なまはげに改称）。
...
### §4-3 確定名と図鑑説明文（v1.3・利用者裁定）

実装は `CAMPUS_PET_FORMS` に `name` / `description` として保持し、選択 UI（名札）と
About ペットずかん（§12-9）の両方がここを単一ソースとして参照する。

| id | 確定名 | 図鑑説明文 |
|---|---|---|
| `robo` | ぴこ | APU-Navi の AI エージェントから生まれた分身。質問がとどくと、耳のフィンを光らせて一緒に考えてくれる。 |
| `sarutahiko` | 猿田彦 | 日本神話で道をひらく“導きの神”。キャンパスを案内するこのアプリの、たのもしい道案内役。 |
| `akita` | こまち | 秋田生まれの秋田犬の子犬。人なつっこくて、こたえを待つ間もしっぽをふって寄りそってくれる。 |
| `gotenmari` | てまりん | 由利本荘のつるし飾り「本荘ごてんまり」から生まれた手まりの妖精。じまんの刺繍でみんなを和ませる。 |
| `namahage` | なまはげ | 秋田の来訪神・なまはげ。こわい顔はやる気の証。なまけ心にカツを入れて、探しものを応援してくれる。 |
| `yatagarasu` | 八咫烏 | 三本足の導きの神使。開発メンバー・小川春翔さんのシステム「八咫烏」から名をうけついだ、稲妻をまとう守り神。 |

- 由来メモ: 猿田彦 = 導きの神（ナビアプリと符合）／八咫烏 = 小川春翔さんのシステム名（利用者指示）／
  ぴこ = AI エージェント／こまち = 秋田犬／てまりん = 由利本荘のごてんまり／なまはげ = 秋田のなまはげ。
- 機能説明の定型文（ずかん冒頭）: 「このコたちに特別な機能はありません。となりで一緒に待ってくれる、癒し担当です。ペットをタップすると、仲間をえらび直せます。」（後半 1 文は v1.4 で追記）
...
- シングルタップ（ドラッグ閾値未満で離す）: その場のリアクション（§12-8 の 3 種からランダム）を再生し、
  **通常送信が可能な状態なら同時に選び直しの顕現を開く**（v1.4・§3-1 ペットタップ経路）。
  `isSending` 等で開けない時はリアクションのみ。
- **ダブルタップの交代召喚は v1.1 で廃止**（ドラッグ操作と競合するため）。
...
8. v1.3: 確定名（猿田彦/なまはげ等）が選択 UI とずかんに反映・About ボタン分岐（仲間なし=ヒント/
   あり=ずかん・`aria-expanded`）・ずかんに機能説明定型文と 6 体の説明（`CAMPUS_PET_FORMS` を単一ソース）。
9. v1.4 ペットタップ経路: タップで `picking` 直行（`waiting` を経ない・一時バブル非表示）＋リアクション併発・
   `isSending`/origin/clarification 中はリアクションのみ・protect-controls 中はタップ自体無効（既存）・
   解除条件と履歴ゼロは合言葉経路と同一。
10. 既存テスト（ChatView/composer 系）が無変更で緑のまま。
...
#### ペットずかん（v1.3・仲間がいる時のボタン押下で表示）

About 本文（キャンパスペット行の後・閉じるボタン行の前）にトグル表示する。カードにせず
境界線＋行の扱い。図（32〜36px の §12-4 SVG・idle）＋確定名＋図鑑説明文（§4-3 が単一ソース）。
**コンテナに必ず `campus-pet-host` を併記**する（--pet-* トークンの定義元。無いと token 参照の
SVG が fill 初期値 = 黒に落ちる）。

```html
<div class="about-pet-guide campus-pet-host">
  ...
  <ul class="about-pet-guide__list">
    <li class="about-pet-guide__row">
      <span class="about-pet-guide__figure" aria-hidden="true"><!-- §12-4 SVG (idle) --></span>
      <span class="about-pet-guide__copy">
        <span class="about-pet-guide__name">ぴこ</span>
        <span class="about-pet-guide__desc">APU-Navi の AI エージェントから生まれた分身。…</span>
      </span>
    </li>
    <!-- ×6（§4-3 の順） -->
  </ul>
...
.about-pet-guide__row {
  display: flex;
  align-items: flex-start;
  gap: 0.625rem;
}

.about-pet-guide__figure {
  width: 2.25rem;
  height: 2.25rem;
  flex: 0 0 auto;
}

.about-pet-guide__figure .campus-pet {
  width: 100%;
  height: 100%;
}

.about-pet-guide__copy {
  display: block;
  min-width: 0;
}

.about-pet-guide__name {
  display: block;
...
### §12-10. 名前（v1.3 で確定 — §4-3 が正）

旧名前案のうち みちかぜ→**猿田彦**・ガオウ→**なまはげ** に改称して利用者確定（2026-07-19）。
確定名と説明文は §4-3 の表が唯一の正で、実装は `CAMPUS_PET_FORMS` に保持する。

### §12-11. 召喚の顕現（v1.3 全面改稿 — 「フォーム骨格」の全廃）

v1.2 のステージも「パネル＋ヘッダ＋フッタ」というフォームの骨格が残り AI 感が拭えず利用者却下。
v1.3 は**カード・枠・背景・見出しアイコンをすべて廃止**し、会話の中に「顕現」だけを置く:
中央の**日輪ハロー**（paper の放射グラデーション・八咫烏の日輪モチーフの引用）が背後に灯り、
**地明かり**（淡い楕円グロー）の上に 6 体が立つ。キャプションは中央寄せの小さな一行のみ。
ホバー/フォーカスで本体が 3px 浮き、足元にシグナル色のスポットライト。現在の仲間は名札が
シグナル色＋スポット常灯。なまはげの★は 2.4s で瞬く。名札は §4-3 の確定名。

出現の振付（reduced-motion では全体 1 段フェードに置換）:

1. 0ms: キャプションがフェードイン（240ms）
2. 0ms: **日輪ハローがブルーム**（scale 0.72→1・460ms expressive）
3. 120ms: **地明かりが横に走る**（scaleX 0.3→1・360ms）
4. 180ms〜: **6 体が左から 70ms 間隔で「ドロン」**（紙色パフ＋ scale 0.55→1.07→1・380ms）
5. 700ms: ヒント/キャンセルがフェードイン（260ms）
  ...
  <div class="campus-pet-picker__apparition">
    ...
    <div class="campus-pet-picker__stage">
      ...
      <button type="button" class="campus-pet-picker__option" data-form="sarutahiko" style="--i: 1">
        ...
        <span class="campus-pet-picker__name">猿田彦</span>
      </button>
      ...
      <button type="button" class="campus-pet-picker__option" data-form="namahage" style="--i: 4">
        ...
        <span class="campus-pet-picker__name">なまはげ</span>
      </button>
...
一時ブロック（召喚シーケンス §3-1）の消滅は従来どおりラッパーの 260ms フェード。顕現は
枠を持たないため、暗い会話背景の上に直接立つ（ハロー・地明かりが台紙の役を担う）。
